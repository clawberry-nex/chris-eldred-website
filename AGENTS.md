# AGENTS.md — chriseldred.co.uk

Project-specific notes for coding-agent sessions in this repo. Read `README.md`
...
Live at https://chriseldred.co.uk. The site is currently public. Optional HTTP
Basic Auth remains available for future review periods through Vercel env vars
`BASIC_AUTH_USER` / `BASIC_AUTH_PASS`; credentials never belong in this repo.
...
`middleware.ts` at the project root is a Vercel Edge Middleware that can gate
every request on HTTP Basic Auth using `BASIC_AUTH_USER` / `BASIC_AUTH_PASS`
env vars. **If either env var is missing the gate is disabled.** Neither is set
in production currently, so the site is public.
...
- **Vercel Hobby plan**: auto-deploys gate on commit author. Resolved
  ...
  (see `~/.agents/AGENTS.md`). If you ever see a deploy stuck in `UNKNOWN`
...
- **Local `.npmrc` has `include=dev`** so build dependencies are installed even
  in shells that inherited `NODE_ENV=production` from a PM2 parent. Don't
  delete it.
